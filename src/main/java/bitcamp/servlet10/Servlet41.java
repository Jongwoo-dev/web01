 ...
 *    </form>
 *    
 * => 멀티파트 방식으로 데이터를 전송하는 경우 HTML 요청 형식 예)
POST /web01/servlet/servlet41 HTTP/1.1
Host: localhost:8080
Connection: keep-alive
Content-Length: 502108
Cache-Control: max-age=0
Origin: http://localhost:8080
Upgrade-Insecure-Requests: 1
User-Agent: Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36
Content-Type: multipart/form-data; boundary=----WebKitFormBoundaryze40YF9R4WvhFLyY
Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*//*;q=0.8
Referer: http://localhost:8080/web01/servlet/servlet41_form.html
Accept-Encoding: gzip, deflate, br
Accept-Language: ko-KR,ko;q=0.8,en-US;q=0.6,en;q=0.4

------WebKitFormBoundaryze40YF9R4WvhFLyY
...
ccc
------WebKitFormBoundaryze40YF9R4WvhFLyY
Content-Disposition: form-data; name="photo"; filename="img01.png"
    ...
    try {
      ...
      for (FileItem item : items) {
        if (item.isFormField()) { // 일반 폼 항목이면,
          ...
          /*
          item.write(new File("C:/workspace/web01/src/main/webapp/upload/"
              + item.getName()));
           */
          // => 웹 애플리케이션이 배치된 폴더에 두어야 한다.
          ServletContext sc = this.getServletContext();
          out.printf("실제 저장될 경로 = %s\n", sc.getRealPath("/upload/" + item.getName()));
          item.write(new File(
