...
HTTP GET 요청
=> request0line의 URI에 데이터를 첨부하여 서버로 보낸다.
=> 프로토콜 예)

GET /web01/servlet/servlet18?data1=aaa&data2=bbb&data3=ccc HTTP/1.1
Host: localhost:8080
Connection: keep-alive
Upgrade-Insecure-Requests: 1
User-Agent: Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.0.2840.99 Safari/537.36
Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*//*;q=0.8
Referer: http://localhost:8080/web01/servlet/servlet18.html
...
메시지 본문
----------
=> GET 요청은 데이터를 uri에 첨부하기 떄문에 메시지 본문을 따로 보내지 않는다
=> POST 요청은 "빈 줄" 다음에 데이터를 첨부하여 보낸다.
=> 프로토콜 예)

POST /web01/servlet/servlet18 HTTP/1.1
Host: localhost:8080
Content-Length: 29
 ...
 *       언제든 해당 요청을 실행할 수 있다.
 * => 웹서버 마다 Request-Line으로 보낼 수 있는 데이터의 크기가 제한된다.
 *    => 많은 양의 데이터를 GET 방식으로 서버에 보낼 수 없다.
 *    => 예) 톰캣 서버(8KB), IIS6/7(16KB) 등
 *    => 조회할 때 사용할 간단한 키 값, 검색할 때 사용할 검색어 정보의 간단한
 *       데이터를 보낼 때 사용한다.
 * => URL에 데이터를 첨부한다.
 *    => 텍스트 데이터만 서버에 보낼 수 있다.
 *    => 물론, Base86와 같은 인코딩을 사용하면 바이너리 데이터(GIF, JPEG 등)를 
 *       문자화 시켜 서버에 바이너리 데이터를 보낼 수 있다.
 *       하지만, 일단 GET 요청에서 URL 크기에 제한이 있기 때문에
 *       바이너리 데이터를 보내는데 사용하지 않는다.
 * => GET 방식이 좋은 점
 ...
 *      즐겨 찾기나 북마크로 현재 페이지를 저장할 수 없다.
 *    
 */

/* GET vs POST 
 * 데이터 전송 형식    => URL에 첨부 vs 메시지 본문에 첨부
 * 보안                => URL에 첨부되기 때문에 웹 브라우저에 임시 보관될 수 있음(보안 취약)
 *                        vs 캐시(cache; 임시 보관)되지 않는다.(보안에 양호)
 * 바이너리 전송 가능  => 불가능 vs 가능
 *   (GET 방식은 Base64와 같은 인코딩을 통해 바이너리를 문자화 시켜 전송 가능)
 * 전송 데이터의 크기  => 8KB 또는 16KB 로 제한됨 vs 제한 없음.
 *   (웹서버 마다 HTTP 요청 라인이나 헤더의 크기를 제한하기 때문)
